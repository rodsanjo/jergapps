...
class contacto extends \core\Controlador {
    ...
    private static $url = 'www.artedetectives.hol.es';
...
//                        \core\Distribuidor::cargar_controlador("usuarios", "form_login", $datos);
        ...
        if (self::enviar_mail_validar($datos) && $validacion_catcha) {
                    ...
                    <body>
                        <div style='text-align:left;'>
                            ...
                            <ul>
                                ...
                                <li>Teléfono de contacto dado: $telefono</li>
                            </ul>
            ...
            if ( $envio_email = mail($to, $subject, $message, $additional_headers))  {
                $datos["mensaje"] = "<p style='float:none'>Su mensaje ha sido enviado con los siguientes datos:</p>$message";
                if(\core\Array_Datos::contenido("recibirCopia", $_REQUEST) == true){    //Enviamos una copia del mensaje al usuario
                    mail($responder_a, $subject, $message."<br/><p style='color:red'>Mensaje enviado desde <a href='http://".self::$url."'>".self::$url."</a></p>", $additional_headers);
                }
            }
            else {
